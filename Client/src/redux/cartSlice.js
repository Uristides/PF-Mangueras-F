...
export const addToCart = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }, { getState }) => {
    try {
      // Fetch product stock
      const { data: productData } = await axios.get(`${backendUrl}/products/${productId}`);
      const productStock = productData.stock;

      const state = getState();
      const cartItem = state.cart.items.find((item) => item.startsWith(`${productId}:`));
      const currentQuantity = cartItem ? parseInt(cartItem.split(":")[1], 10) : 0;
      ...
      const itemInfo = {
        id: userId, // Corrected to userId instead of id
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addToCart.fulfilled, (state, action) => {
        ...
        const cartItemIndex = state.items.findIndex((item) => item.startsWith(`${productId}:`));

        if (cartItemIndex !== -1) {
          const [existingProductId, existingQuantity] = state.items[cartItemIndex].split(":").map(Number);
